...
'''
Ідея програми: створимо граф - спрощену логістичну карту, на якій будуть великі міста країн ЄС,
ребра графа матимуть два параметри - середня швидкість поїздки між заданими містами, та відстань.
Додаємо візуалізацію карти, після перегляду користувач обирає два міста, між якими потрібно прокласти
найкоротший маршрут. Після обчислення маршруту програма виводить його візуалізацію, також прописую загальну
відстань маршруту та час, який піде на цю поїздку.
Обчислення відбуваються за допомогою алгоритма Дейкстри. Я взяв цей алгоритм вже в готовому вигляді,
але детально розібрав його структуру і написав покрокову анотацію-довідку, для можливою роботою з ним в подальшому.

Для реалізації пріоритетної черги використовуємо бібліотеку heapq.
heapq є стандартною бібліотекою Python, яка реалізує пріоритетну чергу (min-heap).
Пріоритетна черга дозволяє ефективно отримувати елемент з найменшим значенням за час O(log n), що є корисним
для алгоритмів, що потребують обробки елементів у порядку їх пріоритету (наприклад, алгоритм Дейкстри або A*).
heapq дозволяє реалізувати пріоритетну чергу на основі списку Python. Він автоматично підтримує порядок елементів
таким чином, що найменший елемент (за значенням) завжди знаходиться на початку списку. Завдяки цьому можна ефективно
отримувати вузол з найменшою відстанню для подальшої обробки, що важливо для алгоритмів типу Дейкстри,
де в кожному кроці вибирається вузол з найменшою поточною відстанню.
Бібліотека heapq реалізує структуру даних купа (heap), зокрема min-heap (мін-купа). Це структура даних,
у якій батьківський елемент завжди має менше значення, ніж його нащадки. Це дозволяє ефективно знаходити елемент
з найменшим значенням (наприклад, вузол з найменшою відстанню) і обробляти його.

Купа — це тип бінарного дерева, де кожен батьківський елемент має більшу або меншу значущість, ніж його нащадки,
в залежності від того, чи це min-heap чи max-heap:

    Min-heap (мін-купа) — батьківський елемент має найменше значення.
    Max-heap (макс-купа) — батьківський елемент має найбільше значення.

У heapq реалізована саме min-heap, тобто елементи упорядковані таким чином, що мінімальний елемент завжди знаходиться
на вершині (найближчий до кореня дерева). Це робить пошук мінімального елемента дуже швидким (O(1)),
а також забезпечує ефективну вставку та видалення елементів (O(log n)).

Основні операції в купі:
    heappush(heap, item): додає елемент в купу, підтримуючи порядок.
    heappop(heap): видаляє мінімальний елемент (корінь купи) та повертає його.
    heapify(iterable): перетворює будь-який ітерабельний об'єкт на купу.
'''
